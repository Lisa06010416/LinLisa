...
public class ChessBoard extends MouseAdapter implements Runnable { //繼承至控制滑鼠的Class
    ...
    static int[] iniChessPosition = {0,1,2,3,4,6,14,16,17,18,19,20};//記錄棋子在棋盤的哪個位置上，被吃掉則值為-1
    static int[] ChessPosition;
        
    ImageIcon background = null;

    public static int[] CurrentBoardArray = {1,1,1,1,1,0,1,0,0,0,0,0,0,0,2,0,2,2,2,2,2};//與後端溝通棋盤用
    ...
    ChessBoard(){   
    	ChessPosition = Arrays.copyOf(iniChessPosition, iniChessPosition.length);//記錄棋子在棋盤的哪個位置上，被吃掉則值為-1
    ...
    public void init(){
    	...
    	for(int i=0;i<12;i++){
    		if(ChessPosition[i]!=iniChessPosition[i]){   			
    			MoveChessNumber = i;
    			if(ChessPosition[i]==-1){
    				SetChessOnBoard(MoveChessNumber,iniChessPosition[i]);
    			}
    			else MoveChess(MoveChessNumber,iniChessPosition[i]);
    		}
    	}  
    }
    ...
    public  void InitializeChessImage() //設置一開始的棋子位置
    {
    	...
    	System.out.println("InitializeChessImage");
    	//0~5號為AI 6~11為player 
    	//0~4號棋
    ...
    private void SelectBlackOrWhite()
    {
    	...
    	Black.addActionListener(new ActionListener() {
    	    public void actionPerformed(ActionEvent e) 
    	    {
    	    	...
    	    	MainFrame.setTitle("人工智慧西瓜棋 - AI先手");
    	    }
    	});
    	...
    	White.addActionListener(new ActionListener() {
    	    public void actionPerformed(ActionEvent e) 
    	    {
    	    	...
    	    	MainFrame.setTitle("人工智慧西瓜棋 - 玩家先手");
    	    }
    	});
    ...
    public void ComputerSetBoardArray(int[] BoardArrayFromBack)
    {
    	...
    	System.out.print("目前的盤面CurrentB : ");
    	for(int i=0 ; i<21 ; i++) //找出新的跟舊的差在哪裡
    	{
    		Minor = CurrentBoardArray[i] - BoardArrayFromBack[i]; 
    		
    		
    			System.out.print(CurrentBoardArray[i]+",");
		...
		Game.MoveChessNumber = MoveChessNumber;
		System.out.println(MoveChessOldPosition+"移動到棋盤"+MoveChessNewPosition);
		Game.MoveChessNewPosition = MoveChessNewPosition;
		//////前端移動棋盤/////
    ...
    public void ComputerSetBoardArray2(int[] BoardArrayFromBack)
    {
    	int Minor;
    	...
    	for(int i=0 ; i<21 ; i++) //找出新的跟舊的差在哪裡
    	{
    		...
    		if(BlackOrWhite=="Black") //AI是白色棋子 //找1變0 0變1的點
    		{
        		if(Minor == -1) //0變1 代表電腦下的點
        		{
        			MoveChessNewPosition = i; //移除棋子的位置
    		...
    		else if(BlackOrWhite=="White")	//AI是黑色棋子 //找2變0 0變2的點
    		{
        		if(Minor == -2) //0變2 代表電腦下的點
        		{
        			MoveChessNewPosition = i; //移除棋子的位置
    	...
    	//找出移動的兩個點後
		System.out.println("電腦移動的棋子為"+MoveChessNumber+"號");
		Game.MoveChessNumber2 = MoveChessNumber;
		System.out.println(MoveChessOldPosition+"移動到棋盤"+MoveChessNewPosition);
		Game.MoveChessNewPosition2 = MoveChessNewPosition;
		//////前端移動棋盤/////
		SetLockPicture(BoardPoint[MoveChessOldPosition].x,BoardPoint[MoveChessOldPosition].y);//設定電腦移動前鎖定圖示
